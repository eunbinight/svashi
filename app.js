...
function renderSuggestions(query) {
  ...
  const filtered = MEMBERS_DATA.filter(m =>
    m.n.toLowerCase().includes(q) || m.e.toLowerCase().includes(q)
  ).slice(0, 6);

  if (filtered.length === 0) {
    list.innerHTML = '<li class="sign-page__suggestion sign-page__suggestion--empty">찾으시는 이름이 없나요?</li>';
    list.classList.add('sign-page__suggestions--open');
    return;
  }

  list.innerHTML = filtered.map(m => `
    <li class="sign-page__suggestion" data-email="${m.e.trim()}">
      <span class="sign-page__suggestion-name">${m.n}</span>
      <span class="sign-page__suggestion-email">${m.e.trim()}</span>
    </li>`).join('');
  list.classList.add('sign-page__suggestions--open');
}
...
async function submitWithEmail(email, maxRetries = 3) {
  ...
  const payload = {
    ...
    t1: state.selections[1]
      ? { name: clean(getLectureById(state.selections[1])?.title), replaceable: state.altOk[1] }
      : null,
    t2: state.selections[2]
      ? { name: clean(getLectureById(state.selections[2])?.title), replaceable: state.altOk[2] }
      : null,
    t3: state.selections[3]
      ? { name: clean(getLectureById(state.selections[3])?.title), replaceable: state.altOk[3] }
      : null,
    t4: state.selections[4]
      ? { name: clean(getLectureById(state.selections[4])?.title), replaceable: state.altOk[4] }
      : null,
    signature: state.signature ? clean(getLectureById(state.signature)?.title) : '',
...
function bindEvents() {
  ...
  document.addEventListener('click', async (e) => {
    ...
    if (suggestion && suggestion.dataset.email) {
      const email = suggestion.dataset.email;
      const member = MEMBERS_DATA.find(m => m.e.trim() === email);
      const input = document.getElementById('signEmail');
      const list = document.getElementById('signSuggestions');
      const errorEl = document.getElementById('signError');
      selectedMemberEmail = email;
      if (input) input.value = member ? `${member.n} (${email})` : email;
    ...
    if (e.target.closest('#btnSign')) {
      ...
      if (!selectedMemberEmail) {
        if (errorEl) errorEl.textContent = '목록에서 이름을 선택해주세요';
        input?.focus();
        return;
      }
  ...
  document.addEventListener('input', (e) => {
    if (e.target.id === 'signEmail') {
      selectedMemberEmail = null; // 수동 편집 시 선택 해제
